/**
 * GST Rates and HSN Code Mapping for Different Product Categories
 * Based on Indian GST regulations (as of 2026)
 * 
 * HSN Code: Harmonized System of Nomenclature
 * Used for systematic classification of goods
 */
...
/**
 * Category to HSN Code and GST Rate Mapping
 * Based on current Indian GST rates
 */
export const CATEGORY_GST_MAPPING: Record<string, CategoryGSTInfo> = {
  ...
  'Cameras': {
    hsn: '8525',
    gstRate: 18,
    description: 'Digital cameras and video equipment'
  },
  ...
  // Fashion - 12% GST (readymade garments)
  'Fashion': {
    hsn: '6203',
    gstRate: 12,
    description: 'Readymade garments and clothing'
  },
  ...
  'Footwear': {
    hsn: '6403',
    gstRate: 12,
    description: 'Footwear (retail sale price > ₹1000)'
  },
  ...
  // Grocery - 5% GST (general rate for packaged food)
  'Grocery': {
    hsn: '2106',
    gstRate: 5,
...
/**
 * Get GST information for a product category
 * @param category - Product category
 * @returns CategoryGSTInfo object with HSN code and GST rate
 */
export const getGSTInfoForCategory = (category: string): CategoryGSTInfo => {
  const gstInfo = CATEGORY_GST_MAPPING[category]
  
  if (!gstInfo) {
    // Default to 18% GST for Electronics if category not found
    // This is a safe fallback for most e-commerce items
    return {
      hsn: '8517',
      gstRate: 18,
 ...
 * @param price - Product price (excluding GST)
 * @param category - Product category
 * @returns GST amount
 */
 ...
 * @param items - Array of items with price, quantity, and category
 * @returns Total GST amount
 */
 ...
 * Useful for generating GST invoices showing tax breakdown
 * @param items - Array of items with price, quantity, and category
 * @returns Object with GST amounts grouped by rate
 */
