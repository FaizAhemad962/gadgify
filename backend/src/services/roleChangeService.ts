...
import { findAllAccountsByEmail } from "../utils/userQueryHelper";
...
export class RoleChangeService {
   ...
   */
  private hasHigherOrEqualRole(userRole: string, targetRole: string): boolean {
   ...
   */
  async grantRoleChangePermission(
    grantedById: string,
    ...
    const usersToGrant = await findAllAccountsByEmail(grantedToEmail);
    ...
    const existingPermission = await prisma.roleChangePermission.findUnique({
    ...
    const permission = await prisma.roleChangePermission.upsert({
      where: { grantedToId: fullUserToGrant.id },
      update: {
        grantedById,
        canChangeRoles: true,
   ...
   */
  async revokeRoleChangePermission(revokedById: string, grantedToId: string) {
    ...
    const permission = await prisma.roleChangePermission.findUnique({
      where: { grantedToId },
    });
    ...
    const revoked = await prisma.roleChangePermission.update({
      where: { grantedToId },
      data: {
        isActive: false,
        deletedAt: new Date(),
   ...
   */
  async canChangeRoles(userId: string): Promise<boolean> {
   ...
   */
  async getAllPermissions(requesterId: string) {
    ...
    const permissions = await prisma.roleChangePermission.findMany({
   ...
   */
  async getUserPermission(userId: string, requesterId: string) {
