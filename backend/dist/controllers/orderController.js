...
const checkLowStockAndAlert = async (items) => {
  if (config_1.config.adminEmail) {
    ...
    if (updatedProducts.length > 0) {
      (0, email_1.sendLowStockAlertEmail)(
        config_1.config.adminEmail,
        updatedProducts,
      ).catch((err) =>
        logger_1.default.error(`Low stock alert email failed: ${err.message}`),
      );
    }
  }
};
...
const retryPayment = async (req, res, next) => {
  try {
    ...
    try {
      ...
      logger_1.default.info(
        `[retryPayment] Razorpay order created: ${razorpayOrder.id}`,
      );
    } catch (razorpayError) {
